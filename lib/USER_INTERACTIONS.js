...
class User {
  ...
  async type(page, selector, text, options = {}) {
    await this._advancedUserType(page, selector, text, options);
  }

  async click(page, selector, options = {}) {
    await this._UserClick(page, selector, options);
  }

  async scroll(page) {
    await this._UserScroll(page);
  }

  async randomAct(page) {
    await this._randomUserAct(page);
  }

  // --- Internal methods ---
  async _UserType(page, selector, text, options = {}) {
    const {
      minDelay = 50,
      maxDelay = 200,
  ...
  async _advancedUserType(page, selector, text, typingStyle = 'normal') {
    const styles = {
    ...
    await this._UserType(page, selector, text, config);
  }
  ...
  async _UserClick(page, selector, options = {}) {
    const {
      missClickChance = 0.3,
