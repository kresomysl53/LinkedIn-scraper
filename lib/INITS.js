...
async function getRandomLowestSession(sessions) {
  ...
  if (sameValueSessions.length > 0) {
    ...
    console.log(`Randomly selected session with the same count: ${sameValueSessions[randomIndex].session_count}`);
    console.log(`Randomly selected session: ${sameValueSessions[randomIndex].proxy}`);
    console.log(`Randomly selected session storage state file: ${sameValueSessions[randomIndex].storage_state_file}`);
    return sameValueSessions[randomIndex];
  }
...
async function saveStorageState(page, filename) {
...
async function loadStorageState(page, filename) {
